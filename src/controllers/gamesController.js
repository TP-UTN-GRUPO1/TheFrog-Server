...
export const createGames = async (req, res) => {
  const { nameGame, platforms, genres, price, imageUrl, developer, rating } =
    req.body;
  ...
  if (!Array.isArray(platforms) || !Array.isArray(genres)) {
    return res
      .status(400)
      .json({ message: "Plataformas y generos deben ser arrays" });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Juego creado exitosamente", game: gameWithDetails });
  } catch (error) {
    console.error("Error al crear el juego:", error);
    res
      .status(500)
      .json({ message: "Error al crear el juego", error: error.message });
  }
};
...
export const deleteGame = async (req, res) => {
  ...
  try {
    ...
    // ✅ Eliminar relaciones many-to-many antes de borrar
    await game.setPlatforms([]); // Borra las relaciones con plataformas
    await game.setGenres([]); // Borra las relaciones con géneros

    await game.destroy(); // Ahora sí se puede eliminar

    res
      .status(200)
      .json({ message: `Juego con ID ${id} eliminado exitosamente` });
  } catch (error) {
...
export const addGameFromArchive = async (res) => {
  if (!dataGames || !Array.isArray(dataGames) || dataGames.length === 0) {
    return res
      .status(400)
      .json({ message: "No se proporcionaron juegos válidos" });
  }
  ...
  try {
    ...
    for (const game of dataGames) {
      for (const field of requiredFields) {
        if (!game[field]) {
          return res
            .status(400)
            .json({ message: `Falta el campo ${field} en un juego` });
        }
      }
      if (!Array.isArray(game.platform) || !Array.isArray(game.genre)) {
        return res
          .status(400)
          .json({ message: "Plataformas y géneros deben ser arrays" });
      }
    }
    ...
    res
      .status(500)
      .json({ message: "Error al crear los juegos", error: error.message });
  }
};
...
export const updateGame = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al actualizar el juego", error: error.message });
  }
};
